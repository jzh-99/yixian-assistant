...
export function toOpportunitySmartModel(result) {
  const fields = result?.fields || {}
  const warnings = result?.warnings || []
  const confidence = Number(result?.confidence ?? 1)
  ...
  return {
    customer: fields.customerName || '',
    name: fields.title || '',
    level: levelMap[fields.intentLevel] || 'B',
    amount: fields.estimatedAmount ? Math.round(Number(fields.estimatedAmount) / 10000) : '',
    nextContact: fromContractDateTime(fields.nextContactTime),
