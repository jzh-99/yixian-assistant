...
| 功能 | 说明 | 前端文件 | 后端接口 |
|------|------|----------|----------|
| 装维工作台 | 四态任务筛选、超时高亮、快速领料入口 | `app/screens/OpsHomeScreen.jsx` | `GET /api/v1/tasks` |
| 接单 | 待接单 → 已接单 | `app/core/taskActions.js` | `POST /api/v1/tasks/{id}/accept` |
| 开始工作 | 已接单 → 进行中 | `app/core/taskActions.js` | `POST /api/v1/tasks/{id}/start` |
| AI 领料填单 | 自然语言描述 → AI 解析 → 确认提交 | `SmartFormScreen.jsx` + `MaterialPreviewScreen.jsx` | `POST /api/v1/ai/extract` |
| 完工反馈 | 用料、备注、图片、签名 | `app/screens/TaskFeedbackScreen.jsx` | `POST /api/v1/tasks/{id}/complete` |
...
1. 复制 `app/screens/` 与 `app/core/` 到目标项目
