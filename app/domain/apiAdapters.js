...
export function applicationFromApi(item, roleCode) {
  const extra = parseJson(item.extraJson, {})
  const typeName = {
  ...
  return {
    ...
    fields,
    timeline: [{
      title: item.status === APPLICATION_STATUS.PENDING ? '申请已提交' : '状态已更新',
      time: displayTime(item.updateTime || item.createTime),
      note: item.status === APPLICATION_STATUS.PENDING ? '等待审批' : '请查看当前状态',
      state: item.status === APPLICATION_STATUS.REJECTED ? 'error' : 'current',
    }],
  }
}
